...
<body>
  ...
  <main class="container mb-5 p-5">
    ...
    <div class="bg-white p-4 rounded shadow-sm mb-4" style="display: none;" id="presentation-form">
      ...
      <form action="<?= BASE_URL ?>pages/admin/musicalScores/actions/create.php" method="post">
    ...
    if (!empty($musicsList)) {
      ...
      foreach ($musicsList as $musicRow) {

        // Dados do partitura
        $musicName = htmlspecialchars($musicRow['music_name'] ?? '', ENT_QUOTES, 'UTF-8');
        $musicId = (int) $musicRow['music_id'] ?? null;

        if ($currentGenre != htmlspecialchars($musicRow['music_genre'] ?? '', ENT_QUOTES, 'UTF-8')) {
          if ($currentGenre != "") {
            echo "</div>";
          }

          $currentGenre = htmlspecialchars($musicRow['music_genre'] ?? '', ENT_QUOTES, 'UTF-8');
          echo "<h2 class='mt-5 border-bottom pb-2 text-primary mb-4'>$currentGenre</h2>";
          echo "<div class='row g-4'>";
        }

        if ($lastName != htmlspecialchars($musicRow['music_name'] ?? '', ENT_QUOTES, 'UTF-8')) { ?>
          <div class='col-12 col-sm-6 col-lg-3'>
            <div class='card musician-card h-100 border-0 shadow-sm'>
              ...
              <a href="<?= BASE_URL ?>pages/admin/musicalScores/edit/index.php?musicId=<?= htmlspecialchars($musicId) ?>"
