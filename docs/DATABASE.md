...
## Recomendacoes de evolucao de schema

1. Adicionar UNIQUE em `musicians.musician_login` para garantir unicidade no banco.
2. Avaliar politica de ON DELETE/ON UPDATE nas FKs conforme regra de negocio (atualmente usa padrao do InnoDB).
3. Considerar indice por data em `presentations.presentation_date` para consultas frequentes por cronograma.
4. Validar tamanho maximo e estrategia de armazenamento de `file` e `profile_image` (nome de arquivo vs URL completa).
